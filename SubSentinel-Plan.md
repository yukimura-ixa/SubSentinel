...
- **Auth**: Use Supabase Auth with email-based magic links for authentication. Sending email is required to deliver magic links to users. This approach is chosen because it is supported on the free tier and avoids complex OAuth flows. Minimal scopes will be stored to enable future Google Calendar integration.
